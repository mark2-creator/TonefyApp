...
2. On-device test of `rebuild/phase-4` is in progress. Latest publish to `preview` is
   ...
   - **Fonts (`f8b0d7ec`)** — the text sheet now offers 130 families instead of
     ...
     reads `fonts/manifest.json` rather than a hardcoded map. **The backend has
     not been restarted, so exports still use the old 21-entry map** — until
     `pm2 restart tonefy-backend`, a font outside those 21 previews correctly and
     exports in ImageMagick's default face. Test: pick a font, check the preview,
     then export and confirm the burned-in text matches.
