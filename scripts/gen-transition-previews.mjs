...
// Regenerating writes new bytes to the SAME url, and the previews are served with a
// 30 day max-age and cached to disk by the picker - so without a version in the url a
// re-render is invisible to every phone that has already seen the old one. This file
// is written on each run and imported by the picker, so publishing the app is what
// busts the cache.
const VERSION_FILE = path.resolve(HERE, '../constants/transitionPreviewVersion.js');
...
async function main() {
  ...
  // Only on a full run: a --only regeneration would otherwise invalidate all 133 for
  // the sake of one, and make every phone re-download the set.
  if (!only) {
    ...
    await writeFile(VERSION_FILE,
      ...
      + `export const TRANSITION_PREVIEW_VERSION = '${stamp}';\n`);
